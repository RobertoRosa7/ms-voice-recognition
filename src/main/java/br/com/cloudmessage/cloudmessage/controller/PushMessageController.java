...
public class PushMessageController {
    ...
    public ResponseEntity<HttpStatus> subscribe(@RequestBody NotificationModel body) {
...
//		this.notificationService.sendNotification(subscribe, notificationDto.serialize());
    ...
    public ResponseEntity<HttpStatus> unsubscribe(@RequestBody NotificationModel body) throws Exception {
    ...
    @GetMapping(value = "")
    public ResponseEntity<HttpStatus> findById(@RequestParam Long id) {

        if (id == null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "client id is required");
        }

        NotificationModel doc = this.notificationService.findByClientId(id);
